...
class NetSelectionDialog(wx.Dialog):
    def __init__(self, parent, nets):
        super().__init__(parent, title="选择网络", style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        ...
        if ok_button:
            ok_button.SetLabel("确认(&O)")
            
        # 修改CANCEL按钮文本
        cancel_button = self.FindWindowById(wx.ID_CANCEL)
        if cancel_button:
            cancel_button.SetLabel("取消(&C)")
...
class PowerTreePanel(wx.Panel):
    ...
    def _init_ui(self):
        ...
        left_sizer.Add(wx.StaticText(left_panel, label="检测电源供电轨"), 0, wx.ALL, 2)

        self.rail_list = wx.ListBox(left_panel, style=wx.LB_SINGLE)
        left_sizer.Add(self.rail_list, 1, wx.EXPAND | wx.ALL, 2)

        self.btn_add_rail = wx.Button(left_panel, label="手工添加网络")
        left_sizer.Add(self.btn_add_rail, 0, wx.EXPAND | wx.ALL, 2)

        # Config buttons
        config_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.btn_save_config = wx.Button(left_panel, label="保存配置")
        self.btn_load_config = wx.Button(left_panel, label="加载配置")
        ...
        # Rail Properties
        prop_sizer = wx.StaticBoxSizer(wx.VERTICAL, self.detail_panel, "供电轨属性")

        h_volt = wx.BoxSizer(wx.HORIZONTAL)
        h_volt.Add(wx.StaticText(prop_sizer.GetStaticBox(), label="额定电压（V）："), 0, wx.ALIGN_CENTER_VERTICAL|wx.RIGHT, 5)
        ...
        self.btn_add_src = wx.Button(self.detail_panel, label="+ 电压源")
        self.btn_add_load = wx.Button(self.detail_panel, label="+ 负载")
        self.btn_add_reg = wx.Button(self.detail_panel, label="+ 调节器")
        self.btn_del_comp = wx.Button(self.detail_panel, label="- 移除")
    ...
    def on_add_component(self, mode):
        ...
        dlg = ComponentSelectorDialog(self, "选择器件", self.active_rail.net_name, available_comps)
        dlg.set_mode(mode)
    ...
    def _edit_source(self, idx):
        ...
        dlg = ComponentSelectorDialog(self, "修改电压源", self.active_rail.net_name, available_comps)
        dlg.set_mode("SOURCE")
    ...
    def _edit_load(self, idx):
        ...
        dlg = ComponentSelectorDialog(self, "修改负载", self.active_rail.net_name, available_comps)
        dlg.set_mode("LOAD")
    ...
    def on_add_regulator(self, event):
        ...
        dlg = RegulatorDialog(self, "添加调节器", rail_names, self.discoverer, input_rail=self.active_rail.net_name)
